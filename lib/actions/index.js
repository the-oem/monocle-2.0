...
export const fetchAllJobs = ((latLng, radius) => {
  const jobPromises = [];
  for (let i = 1; i < 121; i++) {
...
export const filterJobData = (data, latLng, radius) => {
  ...
  const filteredData = data.forEach(page => {
    page.jobs.filter(job => {
      ...
      return isPointInCircle(jobLatLng, latLng, meterRadius);
    });
  });
